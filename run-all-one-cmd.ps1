...
foreach ($svc in $services) {
  $jobs += Start-Job -Name $svc.Name -ScriptBlock {
    param($rootPath, $servicePath, $command)
    Set-Location (Join-Path $rootPath $servicePath)
    Invoke-Expression $command
  } -ArgumentList $root, $svc.Path, $svc.Command
}

Write-Host "Started $($jobs.Count) services in background jobs."
Write-Host 'Use: Get-Job | Format-Table Id, Name, State'
Write-Host 'Use: Receive-Job -Name auth-service -Keep'
